...
static void registry_handle_global(void *data, struct wl_registry *registry,
  ...
  if (strcmp(interface, zwlr_foreign_toplevel_manager_v1_interface.name) == 0) {
    if (state->cmd_type == WLRCTL_COMMAND_TOPLEVEL) {
      ...
      zwlr_foreign_toplevel_manager_v1_add_listener(
          state->ftl_mgr, &zwlr_foreign_toplevel_manager_v1_listener, state);
    }
  }
  ...
  // Bind ext_workspace_manager_v1
  if (strcmp(interface, ext_workspace_manager_v1_interface.name) == 0) {
    if (state->cmd_type == WLRCTL_COMMAND_WORKSPACE) {
      state->workspace_mgr = wl_registry_bind(
          registry, name, &ext_workspace_manager_v1_interface, 1);
    }
  }
  ...
  if (strcmp(interface, ext_foreign_toplevel_list_v1_interface.name) == 0) {
    if (state->cmd_type == WLRCTL_COMMAND_TOPLEVEL) {
      state->ftl_list_mgr = wl_registry_bind(
          registry, name, &ext_foreign_toplevel_list_v1_interface, 1);
      ext_foreign_toplevel_list_v1_add_listener(
          state->ftl_list_mgr, &ext_foreign_toplevel_list_v1_listener, state);
    }
  }
}
...
static bool prepare_command(struct wlrctl *state, int argc, char *argv[]) {
  char *command = argv[0];
  static const struct token commands[] = {
      ...
      {"workspace", WLRCTL_COMMAND_WORKSPACE},
  ...
  case WLRCTL_COMMAND_OUTPUT:
    ...
    break;
  case WLRCTL_COMMAND_WORKSPACE:
    prepare_workspace(state, argc - 1, argv + 1);
    break;
...
int main(int argc, char *argv[]) {
  ...
  const char *usage =
      "Usage: wlrctl [options] [keyboard|pointer|toplevel|workspace] <action>\n"
      "\n"
  ...
  case WLRCTL_COMMAND_POINTER:
    ...
    break;
  case WLRCTL_COMMAND_TOPLEVEL:
    if (!state.ftl_mgr && !state.ftl_list_mgr) {
      die("Foreign Toplevel Management or List interface not found!\n");
    }
    run_toplevel(&state);
    break;
  case WLRCTL_COMMAND_OUTPUT:
    ...
    break;
  case WLRCTL_COMMAND_WORKSPACE:
    if (!state.workspace_mgr) {
      die("Workspace Management interface not found!\n");
    }
    run_workspace(&state);
    break;
  case WLRCTL_COMMAND_UNSPEC:
